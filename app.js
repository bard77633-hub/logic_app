(function() {
    ...
    function createInitialAndCircuit() {
        ...
        const input1 = createComponent('INPUT', 60 + x_offset, 100 + y_offset);
        const input2 = createComponent('INPUT', 60 + x_offset, 220 + y_offset);
    ...
    // --- IDベースの結線作成 (初期配置用) ---
    function createWireFromIDs(fromId, fromTerminal, toId, toTerminal) {
        const id = state.nextId++;
    ...
    function updateDOM() {
        state.components.forEach(comp => {
            ...
            if (comp.type !== 'INPUT' && label) {
                ...
                label.textContent = `${comp.type} (${displayValue})`;
            }
        });
